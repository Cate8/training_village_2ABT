...
"""
    ...
    def create_trial(self):
        ...
        if self.current_trial == 1:
        ...
        elif self.opto_bool == 1:  # OPTO trial: laser ON during ITI + center_led_on, OFF before choice

            # -----------------
            # PARAMETERS
            # -----------------
            CENTER_WINDOW = getattr(self.settings, "center_window", 15.0)
            SIDE_WINDOW   = getattr(self.settings, "side_window",   15.0)
            OMIT_ON_TIME  = 1.0
            ...
            # -------------------------------------------------
            # If center poke occurs → first turn laser OFF, then go to side_led_on.
            # If timeout → omission.
            self.bpod.add_state(
                state_name='c_led_on',
                state_timer=CENTER_WINDOW,
                state_change_conditions={
                    self.ports.center_poke: 'laser_off_before_side_led',
                    Event.Tup: 'omission_laser_off_exit'
                },
                output_actions=[self.ports.LED_c_on] + LASER_ON
            )

            # -------------------------------------------------
            # 3) Center poke made — Laser OFF before choice period
            # -------------------------------------------------
            self.bpod.add_state(
                state_name='laser_off_before_side_led',
                state_timer=0.05,
                state_change_conditions={Event.Tup: 'side_led_on'},
                output_actions=LASER_OFF
            )

            # -------------------------------------------------
            # 4) SIDE WINDOW — Laser OFF
            # -------------------------------------------------
            # The animal makes the choice with the laser already OFF.
            self.bpod.add_state(
                state_name='side_led_on',
                ...
                output_actions=[self.ports.LED_l_on, self.ports.LED_r_on] + LASER_OFF
            )
