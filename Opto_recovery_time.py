...
class Opto_recovery_time(Task):
    ...
    def __init__(self):
        ...
        self.info = """
last condition of light in mPFC with the good lasers and enough power
---------------------------------------------------------------------------------------
...
In this condition animals are expected to show increased recovery times following optogenetic stimulation.
-----------------------------------------------------------------------------------
Task Variables:  
...
"""
    def start(self):
        ...
        self.reward_drunk = 0
        self.settings.lambda_param = 0.1667  # 6 seconds
    ...
    def create_trial(self):
        ...
        self.opto_onset = 0
        self.duration_light = 4

        # IMPORTANT: initialize these variables in every trial
        self.opto_bool = 0
        self.recovery_time = np.nan
        self.iti_long_enough_for_opto = int(self.random_iti > self.duration_light)


        if self.random_iti > self.duration_light:
            if random_number <= 0.51:  # 51% of trial that has ITI > 4s, opto on 25%
                self.opto_bool = 1
                self.recovery_time = self.random_iti - self.duration_light
        ...
        if self.opto_bool == 1:
            ...
            self.bpod.add_state(
                ...
                state_change_conditions={Event.Tup: 'light_onset'},
            ...
            # Optional delay before light onset.
            # Here it is 0, so light starts immediately at the beginning of the ITI.
            self.bpod.add_state(
                state_name='light_onset',
                state_timer=self.opto_onset,
                state_change_conditions={Event.Tup: 'iti_light_on'},
                output_actions=[]
            )

            # 4 s fixed light during the ITI
            self.bpod.add_state(
                state_name='iti_light_on',
                state_timer=self.duration_light,
                state_change_conditions={Event.Tup: 'drink_delay'},
                output_actions=[Output.BNC1High, Output.SoftCode2]
            )

            # Light OFF, but ITI continues.
            # This is the real recovery-time period.
            self.bpod.add_state(
                state_name='drink_delay',
                state_timer=self.recovery_time,
    ...
    def after_trial(self):
        ...
        self.register_value('opto_onset', self.opto_onset)
        self.register_value('duration_light', self.duration_light)
        self.register_value('recovery_time', self.recovery_time)
        self.register_value('opto_condition', 'recovery_time')
        self.register_value('iti_long_enough_for_opto', self.iti_long_enough_for_opto)
