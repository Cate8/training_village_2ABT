...
def plot_lick_raster_with_states_S4(ax, df, fig=None):
    """
    ...
    for i, row in df.iterrows():
        try:
            ...
            bands = [
                (rel('STATE_c_led_on_START'), rel('STATE_c_led_on_END'), 'yellow'),
                (rel('STATE_side_led_on_START'), rel('STATE_side_led_on_END'), 'lightblue'),
                (rel('STATE_drink_delay_START'), rel('STATE_drink_delay_END'), 'orange'),
                (rel('STATE_water_delivery_START'), rel('STATE_water_delivery_END'), 'lightgreen'),
                (rel('STATE_water_delivery_START'), rel('STATE_water_delivery_END'), 'red')  # Optional penalty
            ]
    ...
    ax.scatter(centre_raster_trials, centre_raster_times, marker='_', color='gray', s=40, alpha=0.7, label='Centre')
